...
setwd('./RESULTS_SAMPLE/Snaps_LoopFalse')
################### Loop=FALSE Setting #########################
#ovFile = ''
...
# pdf("Orig_PairPlot.pdf", height=100, width=100)
...
pdf("Train_PairPlot.pdf", height=100, width=100)
par(mfrow=c(nrow, ncol), cex=0.5, mai=c(0.1, 0.1, 0.1, 0.1))
par("mar")
par(mar=c(1,1,1,1))
for(row in 1:ncol)
...
write.table(sim_data, file="SIM_Data", sep=" ", col.names = F, row.names = F)

pdf("SimData_PP.pdf", height=100, width=100)
par(mfrow=c(nrow, ncol), cex=0.5, mai=c(0.1, 0.1, 0.1, 0.1))
par("mar")
par(mar=c(1,1,1,1))
for(row in 1:ncol)
