...
import scipy
from scipy.stats import rankdata, fisk, expon, pareto, lognorm, weibull_min, gamma
...
def get_params(BASE_DIR, params_file):
    tview_loopFalse_params = pickle.load(open(os.path.join(BASE_DIR, params_file), "rb"))
    story_ids = []
    nViews = []
    fisk_params = []
    pbar = ProgressBar()
    ...
    for i in pbar(range(len(tview_loopFalse_params))):
        ...
        nViews.append(tview_loopFalse_params[i][1])
        fisk_params.append(
            [tview_loopFalse_params[i][17][0], tview_loopFalse_params[i][17][1], tview_loopFalse_params[i][17][2]])
...
def generate_Anom_data(anom_mode, num_noise_samples, params):
    ...
    if anom_mode == 1:
        ...
        for i in range(num_noise_samples):
            nViews = int(np.random.uniform(low=100, high=10000))
            views = np.random.uniform(low=params[0], high=params[1], size=nViews)
            views[views>1.0] = 1.0
            anom_views.append(views)
        ...
        for i in range(num_noise_samples):
            nViews = int(np.random.uniform(low=100, high=10000))
            views = 1 - gamma.rvs(params[0], scale=params[1], loc=0.0, size=nViews)
            views[views > 1.0] = 1.0
            views[views < 0.0] = 0.0
            anom_views.append(views)
...
def getTestingData(test_df, use_synth_data, anom_mode):
    testData = np.vstack((test_df.iloc[:,0], test_df.iloc[:,1], test_df.iloc[:,2])).T

    if use_synth_data:
        anomData = generate_synth_data(anom_mode)
        testData = np.vstack((anomData, testData))

    return testData

def get_data(BASE_DIR, use_synth_data=False, anom_mode=None, noise_percentage=None, params=None):
    fisk_params, story_ids, nViews = get_params(BASE_DIR, "Snaps_LoopFalse.pkl")

    df_false = pd.DataFrame(fisk_params)
    orig_params = np.vstack((df_false.iloc[:,0], df_false.iloc[:,1], df_false.iloc[:,2])).T
...
if __name__ == '__main__':
    ...
    BASE_DIR = "RESULTS_SAMPLE/Snaps_LoopFalse"
