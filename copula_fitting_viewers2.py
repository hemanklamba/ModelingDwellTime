...
def get_params_NEW(BASE_DIR, params_file):
    viewer_params = pickle.load(open(os.path.join(BASE_DIR, params_file), "r"))
    ...
    for i in pbar(range(len(viewer_params))):
        params = viewer_params[i][0]
        viewer_ids.append(params[0])
        nViews.append(params[1])
        lognorm_params.append(
            [params[4][0], params[4][1][0],
             params[4][1][1], params[4][1][2],
             params[4][2][0], params[4][2][2]])

    lognorm_params = np.array(lognorm_params)
    return lognorm_params, viewer_ids, nViews

def get_params(BASE_DIR, params_file):
    tview_loopFalse_params = pickle.load(open(os.path.join(BASE_DIR, params_file), "r"))
    story_ids = []
    nViews = []
    fisk_params = []
    pbar = ProgressBar()

    for i in pbar(range(len(tview_loopFalse_params))):
        story_ids.append(tview_loopFalse_params[i][0])
        nViews.append(tview_loopFalse_params[i][2])
        fisk_params.append(
            [tview_loopFalse_params[i][11], tview_loopFalse_params[i][12][0], tview_loopFalse_params[i][12][2]])
...
def impute_and_marginal_computation1(test_params, train_params, train_marginal_fits):
    ...
    for i in pbar(range(test_params.shape[0])):
        ...
        pdfs = []
        for j in range(len(train_marginal_fits)):
            if np.isnan(inp_data[j]):
                pdfs.append(pdf_points[j])
            else:
                value = train_marginal_fits[j].pdf(inp_data[j])
                if np.isnan(value):
                    pdfs.append(pdf_points[j])
                else:
                    pdfs.append(train_marginal_fits[j].pdf(inp_data[j]))
    ...
    for i in pbar(range(test_params.shape[1])):
        cdf_val = train_marginal_fits[i].cdf(test_params[:, i])
...
def get_data(BASE_DIR, use_synth_data=False, anom_mode=None, noise_percentage=None):
    params_file = "viewers.pkl"
    lognorm_params, viewer_ids, nViews = get_params_NEW(BASE_DIR, params_file)

    df_viewer = pd.DataFrame(lognorm_params)
    orig_params = np.vstack((df_viewer.iloc[:,0], df_viewer.iloc[:,1], df_viewer.iloc[:,2],
                            df_viewer.iloc[:,3], df_viewer.iloc[:,4], df_viewer.iloc[:,5])).T

    labels = np.zeros(lognorm_params.shape[0])
    ov_params = orig_params

    print ov_params.shape
    print labels.shape
    return ov_params, viewer_ids, nViews, labels
...
if __name__ == '__main__':
    ...
    BASE_DIR = "RESULTS_SAMPLE/Viewers"
    ...
    viewerIdsfile = "viewerIds.pkl"
    nViewsfile = "nviews.csv"

    print "Getting Overall Data"

    ov_params, viewer_ids, nViews, labels = get_data(BASE_DIR)
    ...
    marginals, impute_cdf_points, impute_pdf_points, train_cdfs, train_pdfs = fit_marginals2(train_params)
    ...
    pickle.dump(viewer_ids, open(os.path.join(BASE_DIR, viewerIdsfile), 'w'))
    ...
    print "Num Nans 2=" + str(np.sum(np.isnan(train_cdfs[:, 2])))
    print "Num Nans 3=" + str(np.sum(np.isnan(train_cdfs[:, 3])))
    print "Num Nans 4=" + str(np.sum(np.isnan(train_cdfs[:, 4])))
    print "Num Nans 5=" + str(np.sum(np.isnan(train_cdfs[:, 5])))

    assert min(train_pdfs[:, 0]) >= 0.0
    assert min(train_pdfs[:, 1]) >= 0.0
    assert min(train_pdfs[:, 2]) >= 0.0
    assert min(train_pdfs[:, 3]) >= 0.0
    assert min(train_pdfs[:, 4]) >= 0.0
    assert min(train_pdfs[:, 5]) >= 0.0

    assert min(train_cdfs[:, 0]) >= 0.0
    assert min(train_cdfs[:, 1]) >= 0.0
    assert min(train_cdfs[:, 2]) >= 0.0
    assert min(train_cdfs[:, 3]) >= 0.0
    assert min(train_cdfs[:, 4]) >= 0.0
    assert min(train_cdfs[:, 5]) >= 0.0

    assert max(train_cdfs[:, 0]) <= 1.0
    assert max(train_cdfs[:, 1]) <= 1.0
    assert max(train_cdfs[:, 2]) <= 1.0
    assert max(train_cdfs[:, 3]) <= 1.0
    assert max(train_cdfs[:, 4]) <= 1.0
    assert max(train_cdfs[:, 5]) <= 1.0

    print "==========++TESTING======================"
    print "Num Nans 0=" + str(np.sum(np.isnan(test_cdfs[:, 0])))
    print "Num Nans 1=" + str(np.sum(np.isnan(test_cdfs[:, 1])))
    print "Num Nans 2=" + str(np.sum(np.isnan(test_cdfs[:, 2])))
    print "Num Nans 3=" + str(np.sum(np.isnan(test_cdfs[:, 3])))
    print "Num Nans 4=" + str(np.sum(np.isnan(test_cdfs[:, 4])))
    print "Num Nans 5=" + str(np.sum(np.isnan(test_cdfs[:, 5])))

    assert min(test_pdfs[:, 0]) >= 0.0
    assert min(test_pdfs[:, 1]) >= 0.0
    assert min(test_pdfs[:, 2]) >= 0.0
    assert min(test_pdfs[:, 3]) >= 0.0
    assert min(test_pdfs[:, 4]) >= 0.0
    assert min(test_pdfs[:, 5]) >= 0.0

    assert min(test_cdfs[:, 0]) >= 0.0
    assert min(test_cdfs[:, 1]) >= 0.0
    assert min(test_cdfs[:, 2]) >= 0.0
    assert min(test_cdfs[:, 3]) >= 0.0
    assert min(test_cdfs[:, 4]) >= 0.0
    assert min(test_cdfs[:, 5]) >= 0.0

    assert max(test_cdfs[:, 0]) <= 1.0
    assert max(test_cdfs[:, 1]) <= 1.0
    assert max(test_cdfs[:, 2]) <= 1.0
    assert max(test_cdfs[:, 3]) <= 1.0
    assert max(test_cdfs[:, 4]) <= 1.0
    assert max(test_cdfs[:, 5]) <= 1.0